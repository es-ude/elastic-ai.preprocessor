...
from elasticai.preprocessing.framing import SettingsFrame, FrameGenerator
...
def build_waveform_lut(bitwidth: int, signed: bool, f_rpt: float, f_wvf: float,
                       do_optimized: bool, type_sig: str= 'sine', module_id: str= '0',
    ...
    assert type_sig.upper() in ['SINE', 'TRI', 'RECT'], "Only 'type' of ['SINE', 'TRI', 'RECT'] are supported"
    ...
    params = {
        ...
        'datatype_cnt':     get_embedded_datatype(np.ceil(np.log2(data_lut.size)), signed=False),
        'datatype_int':     get_embedded_datatype(bitwidth, signed=signed),
        'num_lutsine':      str(data_lut.size),
        'lut_offset':       str(0 if not do_optimized else (0 if signed else (2 ** (bitwidth_mcu - 1)))),
