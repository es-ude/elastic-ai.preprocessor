...
// wire WAIT_CYC;
// assign [WAIT_WIDTH-'d1:0] WAIT_CYC = §{WAIT_CYC};
...
module LUT_WAVEFORM_OPT#(
    ...
    assign LUT_OUT = (SIGNED_OUT) ? ((cnt_phase == 2'd0) ? {1'd0, lut_data_int[sel_adr]} : ((cnt_phase == 2'd1) ? {1'd0, lut_data_int[sel_adr]} : ((cnt_phase == 2'd2) ? {1'd1, -lut_data_int[sel_adr] - {{(BITWIDTH-'d2){1'b0}}, 1'd1}}    : ({1'd1, -lut_data_int[sel_adr] -  {{(BITWIDTH-'d2){1'b0}}, 1'd1}})))) :
                                    ((cnt_phase == 2'd0) ? {1'd1, lut_data_int[sel_adr]} : ((cnt_phase == 2'd1) ? {1'd1, lut_data_int[sel_adr]} : ((cnt_phase == 2'd2) ? {1'd0, {(BITWIDTH-'d1){1'd1}}} - lut_data_int[sel_adr]             : ({1'd0, {(BITWIDTH-'d1){1'd1}}} - lut_data_int[sel_adr]))));
    ...
    always@(posedge CLK_SYS or negedge RSTN) begin
        if(~RSTN) begin
            ...
            state <= (EN_FLAG) ? 1'd1 : ((cnt_wvf_pos == LUTWIDTH-'d1 && cnt_phase == 2'd3 && inc_cnt_pos) ? 1'd0 : state);
            if(inc_cnt_pos && state) begin
                cnt_phase <= cnt_phase + ((cnt_wvf_pos == LUTWIDTH-'d1) ? 2'd1 : 2'd0);
                cnt_wvf_pos <= (cnt_wvf_pos == (LUTWIDTH-'d1)) ? ((cnt_phase == 2'd3) ? ((EN_FLAG) ? 'd1 : 'd0) : 'd1) : cnt_wvf_pos + 'd1;
            end else begin
                cnt_phase <= cnt_phase;
