...
module RAM_WAVEFORM_OPT#(
    ...
    localparam RAM_DATA = {5'd31, 5'd29, 5'd26, 5'd23, 5'd20, 5'd17, 5'd14, 5'd11, 5'd8, 5'd5, 5'd2, 5'd0};
    ...
    assign RAM_OUT = (SIGNED_OUT) ? ((cnt_phase == 2'd0) ? {1'd0, ram_dout} : ((cnt_phase == 2'd1) ? {1'd0, ram_dout} : ((cnt_phase == 2'd2) ? {1'd1, -ram_dout - {{(BITWIDTH-'d2){1'b0}}, 1'd1}}   : ({1'd1, -ram_dout - {{(BITWIDTH-'d2){1'b0}}, 1'd1}})))) :
                                    ((cnt_phase == 2'd0) ? {1'd1, ram_dout} : ((cnt_phase == 2'd1) ? {1'd1, ram_dout} : ((cnt_phase == 2'd2) ? {1'd0, {(BITWIDTH-'d1){1'd1}}} - ram_dout            : ({1'd0, {(BITWIDTH-'d1){1'd1}}} - ram_dout))));
    
    BRAM_SINGLE#(BITWIDTH-'d1, RAMWIDTH) BRAM(
        .CLK_RAM(CLK_SYS),
        .EN(state || RAM_WE),
        .WE(RAM_WE),
        //.PREDEFINED(RAM_DATA),
