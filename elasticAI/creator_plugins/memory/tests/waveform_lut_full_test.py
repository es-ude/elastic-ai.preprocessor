...
from elasticai.creator.testing import cocotb_test_fixture, eai_testbench, CocotbTestFixture


@cocotb.test()
@eai_testbench
async def wvf_lut_read(dut, bitwidth: int, num_params: int, num_trials: int, waveform: list[int], check: list[int]):
    period_clk = 5
...
def test_waveform_lut_full_normal(
    ...
    cocotb_test_fixture.add_srcs_from_package("memory", "verilog/waveform_lut_full.v")
...
def test_waveform_lut_full_ext_data(
    ...
    waveform = [random.randint(a=conv.minimum_as_integer, b=conv.maximum_as_integer) for _ in range(num_params)]
    check = [waveform[0]]
    for _ in range(num_trials):
        check.extend(waveform[1:])
