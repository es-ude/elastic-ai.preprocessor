...
cocotb_settings = dict(
    src_files=['ring_buffer.v', 'windower_evnt.v'],
    top_module_name='WINDOWER_EVNT',
    cocotb_test_module='elasticai.creator_plugins.windower.tests.windower_evnt_tb',
    path2src=Path(test_dut.__file__).parent / 'verilog',
    params={"BITWIDTH": 8, "SAMPLES": 32, "NUM_SHIFT": 4}
)


@cocotb.test()
async def windower_evnt_tb(dut):
    period_clk = 5
