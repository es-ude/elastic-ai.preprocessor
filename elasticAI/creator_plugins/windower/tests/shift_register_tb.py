...
cocotb_settings = dict(
    src_files=['shift_register.v'],
    top_module_name='SHIFT_REGISTER',
    cocotb_test_module='elasticai.creator_plugins.windower.tests.shift_register_tb',
    path2src=Path(test_dut.__file__).parent / 'verilog',
    params={"BITWIDTH": 8, "SAMPLES": 32}
)


@cocotb.test()
async def shift_register_tb(dut):
    period_clk = 5
    print(dir(dut))
    ...
    for idx in range(3 * used_adrwidth):
        ite += 1
        await RisingEdge(dut.CLK_SYS)
        ...
        if ite <= used_adrwidth:
            ...
            assert dut.DATA_OUT.value == data_in_array[(ite-1) % used_adrwidth]
